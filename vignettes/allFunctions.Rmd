...
>>>>>>> e45c85d7d6b80cffd003b01d68396cbf4879e22d
```{r}
...
<<<<<<< HEAD
We can do a lot with this `mating Scene` object. Let's start by focusing on the spatial dimension of the mating scene.
...
str(eKnn)
hist(eKnn[, 3], 40, main = "Distance to 3rd nearst neighbor (m)")
```
...
=======
We can do a lot with this `mating Scene` object. For example, we can look at the spatial (s) dimension with a map. Note that we can use standard R graphical parameters to change the looks of the graph.   
...
>>>>>>> e45c85d7d6b80cffd003b01d68396cbf4879e22d
Distance is a measure of isolation from mates. To characterize mating potential, which is inversely reltaed to distance, we want proximity to mates. For this, we have function `proximity`.
...
Now that we have saved a proximity object, `eProx`, we can visualize it using function `plotPotential`. There are many ways to do it. The defaults show pairwise values of proximity, including a histogram of all pairs, and network diagram of a random subset of 9 individuals and a heatmap of all interactions betwen those those same nine.
...
>>>>>>> e45c85d7d6b80cffd003b01d68396cbf4879e22d
Now we turn from the spatial dimension of the mating scene to the temporal dimension. We can visualize the coordinates of mating activity in time with a mating schedule. The function `plotScene` works just as well in time as in space, we jsut need to specify which dimenstion--use "t" for time. 
...
=======
Just as we can calculate and visualize the spatial distances between all pairs using function `pairDist`, we can do the same in the temporal dimension with function `overlap`.
...
Function `plotScene` is really handy because it enables us to visualize all dimensions of the mating scene (in this data set we only have two dimensions). In the below figure we also hightlight the focal plants we selected above so we can see where and when they participate in mating.
...
Function `matingSummary` calculates many characteristis of the entire mating scene in all three dimensions. Though inthis example dataset we have only dimensions.
