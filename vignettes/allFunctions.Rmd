...
simulatedCoral <- simulateScene(9, sAlleles = 2)
...
We can also investigate the dynamics of a population's mating scene over multiple seasons. Multi-year scenes need to be formatted as lists, with each list element representing one individual mating scene (or one year). If you have a multi-year dataset, function `makeScene` will make it into a multi-year scene automatically using the argument `multiYear = TRUE`. Alternatively, we can simulate several mating scenes and combine them in a list by hand, as in the example below. 
...
```{r, fig.height = 8}
plot3DPotential(list(syncMulti, proxMulti, compatMulti), subject = 'ind', pt.cex = 0.8, sub.ids = c(1,6,12,18,13,24,55,45,60))
```
